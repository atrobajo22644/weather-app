...
export default function Home() {
  ...
  const [loadingPlace, ] = useAtom(loadingCityAtom);

  const { isLoading, error, data, refetch } = useQuery<WeatherData>('repoData', async () => 
  {
    const {data} = await axios.get(`https://api.openweathermap.org/data/2.5/forecast?q=${place}&cnt=56&appid=${process.env.NEXT_PUBLIC_WEATHER_KEY}`);
    return data;
  });
              ...
              <div className="flex gap-4">
                ...
                <Container className="bg-yellow-300 px-6 gap-4 justify-between overflow-x-auto">
                  <WeatherDetails
                    ...
                    sunrise={format(fromUnixTime(data?.city.sunrise ?? 1707048197), "H:mm")}
                    sunset={format(fromUnixTime(data?.city.sunset ?? 1707087960), "H:mm")}
